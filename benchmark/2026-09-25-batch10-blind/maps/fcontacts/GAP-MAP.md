# org.fossify.contacts 1.6.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 25 | 8 | 1×verify, 7×S |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. androidx.appcompat.app.h.F0 `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. androidx.biometric.q$a.b |
...
| keyguard | inert | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. androidx.biometric.y$a.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. org.fossify.commons.extensions.O.X |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites; Kotlin casts it non-null in 1 methods (e.g. org.fossify.commons.extensions.Y.P): a null answer throws there, it is not skipped |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. androidx.compose.ui.platform.L0.a |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. androidx.appcompat.widget.A$a.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>11 call sites, e.g. B1.a0.Q |
...
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. org.fossify.commons.helpers.K.k0 |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. E2.i$a.<init> |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>2 call sites, e.g. m5.s.h |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. p1.g$b.c |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. androidx.appcompat.widget.w.b |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. O2.r$a.a |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>13 call sites, e.g. B1.O$a.c `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>50 call sites, e.g. P4.a.K `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. androidx.appcompat.app.f.N |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. androidx.appcompat.app.v.a |
| notification | supplied | C0 | verify | notification (ANS) | none<br>1 call sites, e.g. p1.k.<init> |
| power | supplied | C0 | verify | powermgr | none<br>2 call sites, e.g. androidx.appcompat.app.h$m.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. org.fossify.commons.extensions.O.U |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. androidx.appcompat.app.h.z0 |
| window | supplied | C0 | verify | window_manager | none<br>8 call sites, e.g. S2.d$a.c `framework/core/java/OHServiceManager.java:96` |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>FileProvider, MyContactsContentProvider, InitializationProvider — probe: `probes/provider-manifest` |
