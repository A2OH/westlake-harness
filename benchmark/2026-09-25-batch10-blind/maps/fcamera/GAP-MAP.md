# org.fossify.camera 1.5.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 25 | 8 | 7×S, 1×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 9 | 9 | 2×S, 6×M, 1×L |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| camera | inert | C4 | M | multimedia/camera_framework | Android CameraManager facade over multimedia/camera_framework<br>4 call sites, e.g. G1.b.<init> |
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. j.y.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. t.o.b |
| fingerprint | null | C5 | S | unmapped | truthful local manager (feature absent)<br>2 call sites, e.g. N2.m.c |
| keyguard | inert | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. t.B.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. N.d.Q |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites; Kotlin casts it non-null in 1 methods (e.g. v6.h.t): a null answer throws there, it is not skipped |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. G1.S.a |
...
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. C3.k.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. S0.d.a |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. O3.f.<init> |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. E.d.h |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. N.d.q |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. N2.m.get |
| display | supplied | C0 | verify | display_manager | none<br>3 call sites, e.g. androidx.camera.view.PreviewView.getDisplayManager |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>12 call sites, e.g. A3.d.run `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>44 call sites, e.g. M4.k.g `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. d3.f.run |
| location | supplied | C0 | verify | location | none<br>2 call sites, e.g. b6.e.<init> |
| notification | supplied | C0 | verify | notification (ANS) | none<br>1 call sites, e.g. r2.d.<init> |
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. j.u.<init> |
| role | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. N.d.Q |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.y.C |
| window | supplied | C0 | verify | window_manager | none<br>9 call sites, e.g. F3.d.a `framework/core/java/OHServiceManager.java:96` |
...
- Java absences excluded by API level: {'absent-from-platform': 1, 'newer-than-reference': 6}.
