# com.bnyro.translate 19.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 8 | 1 | 1×verify |
| Keystore & crypto providers | JCA provider names the code selects → providers the Westlake runtime installs → OH HUKS | 0 | 0 | — |
| Package manager & manifest | manifest features and PackageManager calls → Westlake PM semantics | 3 | 2 | 1×verify, 1×M |
...
| Widgets | hollow-candidate | C9 | verify | none (library code inside Westlake) | check each hollow body against AOSP<br>android.widget.AutoCompleteTextView.onFinishInflate, android.widget.FrameLayout.onSizeChanged, android.widget.TextView.onDetachedFromWindow |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.compose.ui.platform.AndroidCompositionLocals_androidKt, kotlin.reflect.jvm.internal.ReflectionFactoryImpl, org.bouncycastle.jsse.provider.BouncyCastleJsseProvider |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. nf.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>2 call sites, e.g. o5.<init> |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>1 call sites, e.g. com.bnyro.translate.App.onCreate |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. sk2.a |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>2 call sites, e.g. b1.a |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>10 call sites, e.g. yf.onDetachedFromWindow `framework/core/java/OHServiceManager.java:105` |
| notification | supplied | C0 | verify | notification (ANS) | none<br>1 call sites, e.g. cg1.<init> |
| window | supplied | C0 | verify | window_manager | none<br>8 call sites, e.g. cl.b `framework/core/java/OHServiceManager.java:96` |
...
| libtesseract.so → 1 classes, 0 members | supplied | C0 | none | through the Java framework | none<br>e.g. java/lang/String |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Create lnk_file in app data | denied | C5 | M | SELinux u:r:normal_hap:s0 → u:object_r:appdat:s0 | neverallow'd for hap_domain on app data; needs a libc-level emulation of symlink(), not a policy rule<br>libtesseract.so:symlink `sepolicy/base/public/hap_domain.te neverallow hap_domain ~{ tmpfs_data_file dev_file ... data_user_file hmdfs ... }:lnk_file *` |
