# org.fossify.voicerecorder 1.7.1 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 24 | 9 | 1×verify, 8×S |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 7 | 7 | 1×S, 4×M, 2×L |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. j.b0.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. r.p.b |
...
| keyguard | inert | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. r.f0.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. w7.a.p0 |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites, e.g. lb.a0.D |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>1 call sites, e.g. w7.a.o0 `framework/package-manager/java/OHUserManager.java:82` |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. a2.f1.a |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. o.q0.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>10 call sites, e.g. a2.d0.<init> |
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. a2.d0.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. ab.j.invoke |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. a5.w.b |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. r1.c.h |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. a2.i.<init> |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. h4.o.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>12 call sites, e.g. a2.b0.run `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>36 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. j.l.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. j.b0.y |
| notification | supplied | C0 | verify | notification (ANS) | none<br>3 call sites, e.g. l3.j.<init> |
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. j.x.<init> |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.b0.C |
| window | supplied | C0 | verify | window_manager | none<br>7 call sites, e.g. a2.a2.a `framework/core/java/OHServiceManager.java:96` |
