# com.github.andreyasadchy.xtra 2.60.3 → OpenHarmony: API shim gap map

Provider: Westlake `corpus2-fixes` @ `6c300b3bd0`; OH board: OpenHarmony 6.1.0.31, arm64, SELinux enforcing policy as loaded. Target SDK 37.

## Summary

| Category | How it reaches OH | Rows | Gaps | Effort profile |
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 14 | 14 | 11×verify, 3×S |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 26 | 9 | 2×verify, 4×S, 3×M |
...
| Legacy HTTP | missing | C1/C5 | S | none (library code inside Westlake) | port from AOSP, or confirm the caller tolerates absence<br>android.net.http.HttpEngine$Builder.setProxyOptions |
...
| WebView | hollow-candidate | C9 | verify | web (ArkWeb) or bundled Chromium | check each hollow body against AOSP<br>android.webkit.WebSettings.setForceDark, android.webkit.WebViewClient.onReceivedError, android.webkit.WebViewClient.shouldOverrideUrlLoading |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | M | audio_framework | replace the hollow binder with an implementation over audio_framework<br>4 call sites, e.g. androidx.activity.ComponentActivity$$ExternalSyntheticLambda11.run `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
...
| device_policy | null | C5 | S | unmapped | truthful local manager (feature absent)<br>3 call sites, e.g. com.github.andreyasadchy.xtra.ui.main.MainActivity$onCreate$1.invokeSuspend |
| media_metrics | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. androidx.work.WorkerKt$$ExternalSyntheticLambda2.run$androidx$media3$exoplayer$ExoPlayerImpl$Api31$$ExternalSyntheticLambda0 |
| phone | inert | C4 | S | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>2 call sites, e.g. androidx.activity.ComponentActivity$$ExternalSyntheticLambda11.run |
...
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. androidx.activity.Api26Impl.getTextClassifier `SystemServiceRegistry.java:413` |
...
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>6 call sites, e.g. androidx.room.RoomDatabase$Builder.build |
...
| appops | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. androidx.core.app.NavUtils.checkSelfPermission |
...
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>24 call sites, e.g. androidx.activity.ComponentActivity$$ExternalSyntheticLambda11.run |
| display | supplied | C0 | verify | display_manager | none<br>3 call sites, e.g. androidx.media3.common.util.Util.getCurrentDisplayModeSize |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>7 call sites, e.g. androidx.core.app.NotificationManagerCompat.<init> |
| power | supplied | C0 | verify | powermgr | none<br>6 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl$AutoTimeNightModeManager.<init> |
...
| wifi | supplied | C0 | verify | communication/wifi | none<br>1 call sites, e.g. com.github.andreyasadchy.xtra.ui.player.MediaPlayerService.create$1 |
...
- Java absences excluded by API level: {'absent-from-platform': 7, 'newer-than-reference': 10}.
