...
Provider: Westlake `HEAD` @ `f6dc615029`; OH board: OpenHarmony 6.1.0.31, arm64, SELinux enforcing policy as loaded. Target SDK 36.
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 33 | 24 | 3×verify, 12×S, 8×M, 1×L |
| Keystore & crypto providers | JCA provider names the code selects → providers the Westlake runtime installs → OH HUKS | 1 | 1 | 1×M |
...
| Native loading & packaging | how the libraries are packaged → what the OH linker can map | 2 | 1 | 1×XS |
...
Against the provider state the app actually ran on, of 5 observed blockers: **5** predicted.

| Blocker | Symptom on device | Row | Outcome |
|---|---|---|---|
| B1 | MainApplication.onCreate: libreactnative.so fails to relocate _ZNSt6__ndk119__shared_weak_count14__release_weakEv; its DT_NEEDED libc++_shared.so resolved to /system/lib64/libc++_shared.so, which the child's search path puts before the app's library directory | `load:shadowed-by-board` (missing) | predicted |
| B2 | JS fatal while the bundle loads: 'Exception in HostFunction: AndroidKeyStore not found' (expo-secure-store opens KeyStore.getInstance("AndroidKeyStore")); ReactHost destroys the instance, the window stays empty | `jca:AndroidKeyStore` (missing) | predicted |
| B3 | 'null cannot be cast to non-null type android.app.UiModeManager' in a JS host function (React Native AndroidInfoModule.uiMode); a module fails to initialise, ExpoRoot throws 'Cannot read property initScreensFeatureFlags of undefined', the app shows its native 'Something went wrong' dialog | `svc:uimode` (null) | predicted |
| B4 | WebView start-up: UserManager.getApplicationRestrictions reaches the in-process IUserManager, which throws UnsupportedOperationException inside a JNI callback; Chromium aborts ([FATAL:jni_android.cc(315)], SIGTRAP) | `svc:user` (strict) | predicted |
| B5 | WebView binds its sandboxed renderer (org.chromium.content.app.SandboxedProcessService0); direct launch cannot start it and Chromium's main thread traps (SIGTRAP on CrBrowserMain) | `wv:renderer-process` (missing) | predicted |
...
| alarm | null | C4 | M | time_service / reminder_agent | Android AlarmManager facade over time_service / reminder_agent<br>8 call sites; Kotlin casts it non-null in 4 methods (e.g. bj.g.<init>, com.braze.managers.m.a, com.braze.managers.u0.<init>): a null answer throws there, it is not skipped |
...
| clipboard | null | C4 | M | miscservices/pasteboard | Android ClipboardManager facade over miscservices/pasteboard<br>4 call sites; Kotlin casts it non-null in 1 methods (e.g. com.facebook.react.modules.clipboard.ClipboardModule.getClipboardService): a null answer throws there, it is not skipped |
...
| power | null | C4 | S | powermgr | Android PowerManager facade over powermgr<br>11 call sites; Kotlin casts it non-null in 2 methods (e.g. com.braze.push.BrazeNotificationUtils.wakeScreenIfAppropriate, q3.x.b): a null answer throws there, it is not skipped |
| uimode | null | C4 | S | display / theme | Android UiModeManager facade over display / theme<br>8 call sites; Kotlin casts it non-null in 2 methods (e.g. com.braze.push.BrazeNotificationUtils.wakeScreenIfAppropriate, com.facebook.react.modules.systeminfo.AndroidInfoModule.uiMode): a null answer throws there, it is not skipped |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>4 call sites, e.g. I0.p$a.a `framework/package-manager/java/OHUserManager.java:46` |
...
| vibrator_manager | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>2 call sites; Kotlin casts it non-null in 1 methods (e.g. Ch.b.B): a null answer throws there, it is not skipped `SystemServiceRegistry.java:796` |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Android keystore provider ("AndroidKeyStore") | missing | C4 | M | security/huks (OH Universal Keystore); keystore2 has no OH counterpart | install an "AndroidKeyStore" provider whose keys come from KeyGenParameterSpec and stay per app: software keys in app data (days), or OH HUKS for hardware-backed keys (weeks). A blocker wherever the app opens it at startup (secure storage, encrypted preferences, biometric crypto, attestation)<br>4 classes name it, e.g. Lgc/N;, Lgc/P;, Lnj/i$v;, Lp/i;; calls KeyGenerator.getInstance("AES"), KeyPairGenerator.getInstance("EC"), KeyStore.getInstance("AndroidKeyStore") |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Packaged libraries a board library of the same name shadows (libc++_shared.so) | missing | C3 | XS | OH dynamic linker search order: /system/lib64/libc++_shared.so comes before the app's library directory | load these 20 libraries in the isolated Android namespace so DT_NEEDED picks the APK's copy (e.g. NDK libc++_shared is std::__ndk1; OH's is not): libNitroMmkv.so libNitroModules.so libappmodules.so libc++_shared.so libexpo-modules-core.so libexpo-updates.so libfbjni.so libgesturehandler.so libhermestooling.so libhermesvm.so libjsi.so libmmkv.so libreact_codegen_RNKC.so libreact_codegen_rnscreens.so libreact_codegen_rnsvg.so libreact_codegen_safeareacontext.so libreactnative.so libreanimated.so librnscreens.so libworklets.so<br>20 APK libraries reach them through DT_NEEDED `manifest/tools/probe_source_app.py:35` |
