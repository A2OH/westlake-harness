...
Provider: Westlake `burgerking-startup-fixes` @ `9b8b86182c`; OH board: OpenHarmony 6.1.0.31, arm64, SELinux enforcing policy as loaded. Target SDK 36.
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 33 | 20 | 3×verify, 10×S, 6×M, 1×L |
...
Of 5 blockers already hit on the board: **3** open, **2** closed per source (confirm on device).

| Blocker | Symptom on device | Row | Status |
|---|---|---|---|
| B1 | MainApplication.onCreate: libreactnative.so fails to relocate _ZNSt6__ndk119__shared_weak_count14__release_weakEv; its DT_NEEDED libc++_shared.so resolved to /system/lib64/libc++_shared.so, which the child's search path puts before the app's library directory | `load:shadowed-by-board` (missing) | open |
| B2 | JS fatal while the bundle loads: 'Exception in HostFunction: AndroidKeyStore not found' (expo-secure-store opens KeyStore.getInstance("AndroidKeyStore")); ReactHost destroys the instance, the window stays empty | `jca:AndroidKeyStore` (supplied) | closed per source (confirm on device) |
| B3 | 'null cannot be cast to non-null type android.app.UiModeManager' in a JS host function (React Native AndroidInfoModule.uiMode); a module fails to initialise, ExpoRoot throws 'Cannot read property initScreensFeatureFlags of undefined', the app shows its native 'Something went wrong' dialog | `svc:uimode` (supplied) | closed per source (confirm on device) |
| B4 | WebView start-up: UserManager.getApplicationRestrictions reaches the in-process IUserManager, which throws UnsupportedOperationException inside a JNI callback; Chromium aborts ([FATAL:jni_android.cc(315)], SIGTRAP) | `svc:user` (strict) | open |
| B5 | WebView binds its sandboxed renderer (org.chromium.content.app.SandboxedProcessService0); direct launch cannot start it and Chromium's main thread traps (SIGTRAP on CrBrowserMain) | `wv:renderer-process` (missing) | open |
...
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>4 call sites, e.g. I0.p$a.a `framework/package-manager/java/OHUserManager.java:72` |
...
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>8 call sites, e.g. androidx.work.impl.background.systemalarm.a.b |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>4 call sites, e.g. androidx.appcompat.widget.k.b |
...
| power | supplied | C0 | verify | powermgr | none<br>11 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl$n.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>3 call sites, e.g. expo.modules.quickactions.a.G |
| uimode | supplied | C0 | verify | display / theme | none<br>8 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.C0 |
