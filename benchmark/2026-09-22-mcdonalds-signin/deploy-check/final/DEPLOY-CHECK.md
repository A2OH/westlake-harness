# Deployment check: McDonald's sign-in build (westlake f6dc615)

304 files staged; the runtime asks for 39 libraries by name.

**Missing: 3. Older than their source: 2.**
...
Matching their source: `libandroid_native_network_compat.so`, `liboh_hwui_shim.so`, `liboh_ime_helper_capi.so`, `libsoundpool.so`, `libwestlake_bionic.so`, `webview-t-lib/libwebview_bionic_shim.so`.
