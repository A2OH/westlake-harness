# com.inspiredandroid.linuxcommandbibliotheca 4.8.0 → OpenHarmony: API shim gap map
...
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 8 | 8 | 7×verify, 1×S |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 12 | 2 | 1×verify, 1×S |
...
| Other Android | hollow-candidate | C9 | verify | unmapped | check each hollow body against AOSP<br>android.animation.Animator.cancel, android.animation.Animator.end, android.animation.AnimatorListenerAdapter.onAnimationStart |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. je.t `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>2 call sites, e.g. kf.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>5 call sites, e.g. androidx.recyclerview.widget.RecyclerView.<init> |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. io.m |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>2 call sites, e.g. bs0.n |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>8 call sites, e.g. uf.onDetachedFromWindow `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>21 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. sd.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. je.x |
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. fe.<init> |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. je.B |
| window | supplied | C0 | verify | window_manager | none<br>14 call sites, e.g. c50.f `framework/core/java/OHServiceManager.java:96` |
...
| Content providers installed at bind (2, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>AndroidContextProvider, InitializationProvider — probe: `probes/provider-manifest` |
