# com.jarsilio.android.scrambledeggsif 1.8.1 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 15 | 2 | 1×verify, 1×S |
...
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>2 call sites, e.g. com.jarsilio.android.scrambledeggsif.CleanUpAlarmReceiver.onReceive |
...
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>9 call sites, e.g. androidx.activity.ImmLeaksCleaner.onStateChanged `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>39 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>1 call sites, e.g. androidx.core.app.NotificationManagerCompat.<init> |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>FileProvider, LogProvider, InitializationProvider — probe: `probes/provider-manifest` |
