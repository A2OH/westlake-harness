# com.celzero.bravedns v0.5.7 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 25 | 8 | 2×verify, 5×S, 1×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 6 | 6 | 4×M, 2×L |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.sharetarget.ShortcutInfoCompatSaverImpl, com.bumptech.glide.GeneratedAppGlideModuleImpl, com.google.crypto.tink.shaded.protobuf.DescriptorMessageInfoFactory |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>3 call sites, e.g. androidx.work.Worker$$ExternalSyntheticLambda0.invoke `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2189` |
...
| download | inert | C4 | S | miscservices/download_server | Android DownloadManager facade over miscservices/download_server<br>3 call sites, e.g. com.celzero.bravedns.download.AppDownloadManager.cancelAndroidDownloadManagerDownloads |
...
| connectivity_diagnostics | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. com.celzero.bravedns.service.DiagnosticsManager.register `ConnectivityFrameworkInitializer.java:52` |
...
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>8 call sites, e.g. androidx.room.RoomDatabase$Builder.build |
...
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>18 call sites, e.g. androidx.appcompat.widget.AppCompatEditText.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>11 call sites, e.g. androidx.navigation.internal.NavContext.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>18 call sites, e.g. androidx.activity.ImmLeaksCleaner.onStateChanged `framework/core/java/OHServiceManager.java:105` |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>33 call sites, e.g. androidx.core.app.NotificationManagerCompat.<init> |
...
| shortcut | supplied | C0 | verify | unmapped | none<br>4 call sites, e.g. androidx.core.os.BuildCompat$Api30Impl.getDynamicShortcuts |
| uimode | supplied | C0 | verify | display / theme | none<br>5 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.mapNightMode |
| wifi | supplied | C0 | verify | communication/wifi | none<br>2 call sites, e.g. com.celzero.bravedns.service.ConnectionMonitor.onVpnStart |
| window | supplied | C0 | verify | window_manager | none<br>11 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.closePanel `framework/core/java/OHServiceManager.java:96` |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| lib/arm64-v8a/libgojni.so → 1 classes, 4 members | supplied | C0 | none | through the Java framework | none<br>e.g. java/lang/Throwable |
