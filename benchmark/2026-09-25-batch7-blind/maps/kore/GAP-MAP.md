# org.xbmc.kore v3.1.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 18 | 6 | 1×verify, 5×S |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.sharetarget.ShortcutInfoCompatSaverImpl, kotlin.reflect.jvm.internal.ReflectionFactoryImpl, kotlinx.coroutines.test.internal.TestMainDispatcherFactory |
...
| download | inert | C4 | S | miscservices/download_server | Android DownloadManager facade over miscservices/download_server<br>2 call sites, e.g. org.xbmc.kore.utils.FileDownloadHelper$1.onSuccess |
| phone | inert | C4 | S | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>1 call sites, e.g. org.xbmc.kore.service.CallStateListener.<init> |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. org.xbmc.kore.utils.UIUtils.handleVibration |
| wifi | null | C4 | S | communication/wifi | Android WifiManager facade over communication/wifi<br>2 call sites, e.g. org.xbmc.kore.ui.sections.localfile.HttpApp.getIpAddress |
...
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. org.xbmc.kore.ui.sections.hosts.AddHostFragmentZeroconf.isNetworkConnected |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>7 call sites, e.g. com.google.android.material.internal.ViewUtils.getInputMethodManager `framework/core/java/OHServiceManager.java:105` |
...
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl$AutoBatteryNightModeManager.<init> |
| uimode | supplied | C0 | verify | display / theme | none<br>2 call sites, e.g. com.google.android.material.color.DynamicColors.getSystemContrast |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>MediaProvider, InitializationProvider, PicassoProvider — probe: `probes/provider-manifest` |
| Component lookups return manifest <meta-data> | supplied | C0 | verify | none (answered from the APK inside Westlake) | apply updateFlagsForComponent semantics in the source-app PM path<br>2 components carry meta-data (0 directBootAware, e.g. ); app calls ['getActivityInfo', 'getPackageInfo', 'getProviderInfo', 'getServiceInfo'] `framework/package-manager/java/SourcePackageRegistry.java:77` — probe: `probes/service-metadata` |
