# com.gh4a 4.6.15 → OpenHarmony: API shim gap map

Provider: Westlake `corpus2-fixes` @ `2b21e1200d`; OH board: OpenHarmony 6.1.0.31, arm64, SELinux enforcing policy as loaded. Target SDK 33.
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 23 | 8 | 2×verify, 5×S, 1×M |
...
| download | inert | C4 | S | miscservices/download_server | Android DownloadManager facade over miscservices/download_server<br>1 call sites, e.g. com.gh4a.utils.DownloadUtils.enqueueDownload |
...
| display | supplied | C0 | verify | display_manager | none<br>4 call sites, e.g. androidx.core.content.ContextCompat$Api30Impl.getDisplayOrDefault |
...
| layout_inflater | supplied | C0 | verify | unmapped | none<br>93 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>11 call sites, e.g. androidx.browser.trusted.TrustedWebActivityService.onAreNotificationsEnabled |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>InitializationProvider, BookmarksProvider, SuggestionsProvider — probe: `probes/provider-manifest` |
...
- 26 service requests use computed names and are not resolved statically.
- Java absences excluded by API level: {'absent-from-platform': 16, 'newer-than-reference': 1}.
