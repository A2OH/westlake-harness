# it.niedermann.owncloud.notes 35.0.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 27 | 8 | 2×verify, 5×S, 1×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 3 | 3 | 3×M |
...
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>2 call sites, e.g. it.niedermann.owncloud.notes.LockedActivity.askToUnlock |
...
| account | supplied | C0 | verify | account/os_account | none<br>14 call sites, e.g. com.owncloud.android.lib.common.operations.RemoteOperation.run |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>7 call sites, e.g. androidx.core.content.pm.ShortcutManagerCompat.getIconDimensionInternal |
...
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>9 call sites, e.g. coil3.network.ConnectivityCheckerKt.ConnectivityChecker |
...
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>24 call sites, e.g. androidx.appcompat.widget.AppCompatTextView.onDetachedFromWindow `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>87 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| power | supplied | C0 | verify | powermgr | none<br>3 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl$AutoBatteryNightModeManager.<init> |
| search | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. it.niedermann.owncloud.notes.share.NoteShareActivity.lambda$initializeArguments$4 |
...
- Java absences excluded by API level: {'absent-from-platform': 42, 'newer-than-reference': 19}.
