# org.proninyaroslav.libretorrent 4.1.1 → OpenHarmony: API shim gap map

Provider: Westlake `corpus2-fixes` @ `2b21e1200d`; OH board: OpenHarmony 6.1.0.31, arm64, SELinux enforcing policy as loaded. Target SDK 35.

## Summary

| Category | How it reaches OH | Rows | Gaps | Effort profile |
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 9 | 9 | 8×verify, 1×S |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 23 | 7 | 1×verify, 5×S, 1×M |
...
| Other | hollow-candidate | C9 | verify | unmapped | check each hollow body against AOSP<br>org.xml.sax.helpers.DefaultHandler.characters, org.xml.sax.helpers.DefaultHandler.endDocument, org.xml.sax.helpers.DefaultHandler.endElement |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>4 call sites, e.g. androidx.work.impl.WorkManagerImpl$$ExternalSyntheticLambda0.invoke `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2189` |
...
| wifi | null | C4 | S | communication/wifi | Android WifiManager facade over communication/wifi<br>2 call sites, e.g. org.proninyaroslav.libretorrent.receiver.SchedulerReceiver.onStartApp |
...
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>9 call sites, e.g. androidx.appcompat.widget.AppCompatEditText.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>5 call sites, e.g. androidx.graphics.shapes.Morph.<init> |
...
| layout_inflater | supplied | C0 | verify | unmapped | none<br>63 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| libtorrent4j.so → 9 classes, 9 members | supplied | C0 | none | through the Java framework | none<br>e.g. java/io/IOException, java/lang/ArithmeticException, java/lang/IllegalArgumentException, java/lang/IllegalStateException |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Create lnk_file in app data | denied | C5 | M | SELinux u:r:normal_hap:s0 → u:object_r:appdat:s0 | neverallow'd for hap_domain on app data; needs a libc-level emulation of symlink(), not a policy rule<br>libtorrent4j.so:symlink `sepolicy/base/public/hap_domain.te neverallow hap_domain ~{ tmpfs_data_file dev_file ... data_user_file hmdfs ... }:lnk_file *` |

## Limits of this map

- 4 service requests use computed names and are not resolved statically.
- Java absences excluded by API level: {'absent-from-platform': 1, 'newer-than-reference': 7}.
