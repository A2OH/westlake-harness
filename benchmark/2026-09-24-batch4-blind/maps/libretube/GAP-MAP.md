# com.github.libretube 32.1 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 27 | 9 | 2×verify, 3×S, 4×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 3 | 3 | 1×S, 1×M, 1×L |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | M | audio_framework | replace the hollow binder with an implementation over audio_framework<br>5 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.dispatchKeyEvent `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>3 call sites, e.g. androidx.room.RoomDatabase$$ExternalSyntheticLambda1.invoke `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2162` |
| phone | inert | C4 | M | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>4 call sites, e.g. com.github.libretube.helpers.PreferenceHelper.getTrendingRegion |
...
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>1 call sites, e.g. androidx.room.TransactorKt.isUserUnlocked `framework/package-manager/java/OHUserManager.java:82` |
| wifi | null | C4 | S | communication/wifi | Android WifiManager facade over communication/wifi<br>1 call sites, e.g. androidx.core.provider.CallbackWrapper.updateWifiLock |
| captioning | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>4 call sites, e.g. androidx.media3.exoplayer.MediaPeriodHolder.selectTracks `SystemServiceRegistry.java:337` |
...
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>17 call sites, e.g. androidx.core.view.ViewCompat.notifyViewAccessibilityStateChangedIfNeeded |
...
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. androidx.work.TracerKt.checkSelfPermission |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. androidx.work.TracerKt.maybeHandleMenuActionViaPerformReceiveContent |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>11 call sites, e.g. androidx.room.Room.isEnabled |
| display | supplied | C0 | verify | display_manager | none<br>4 call sites, e.g. androidx.core.view.WindowCompat$Api30Impl.getDisplayOrDefault |
...
| layout_inflater | supplied | C0 | verify | unmapped | none<br>82 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.ensureSubDecor `SystemServiceRegistry.java:593` |
...
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. androidx.navigation.NavArgsLazy.getInstance |
...
| power | supplied | C0 | verify | powermgr | none<br>4 call sites, e.g. com.github.libretube.ui.fragments.PlayerFragment.onPause |
| shortcut | supplied | C0 | verify | unmapped | none<br>3 call sites, e.g. com.github.libretube.ui.activities.MainActivity.loadIntentData |
| storage | supplied | C0 | verify | filemanagement/storage_service | none<br>1 call sites, e.g. com.github.libretube.LibreTubeApp.onCreate |
