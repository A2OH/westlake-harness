# io.github.muntashirakon.AppManager 4.1.1 → OpenHarmony: API shim gap map
...
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 14 | 14 | 7×verify, 7×S |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 29 | 12 | 2×verify, 8×S, 2×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 6 | 6 | 1×S, 3×M, 2×L |
...
| App framework | missing | C4 | S | ability_runtime | implement the members over ability_runtime<br>android.app.AppOpsManager.MIUI_OP_END, android.app.AppOpsManager.MIUI_OP_START, android.app.AppOpsManager.opToDefaultMode |
| Package manager | missing | C4 | S | bundle_framework | implement the members over bundle_framework<br>android.content.pm.ApplicationInfo.seinfo, android.content.pm.PackageInfo.isStaticOverlay, android.content.pm.PackageInfo.overlayFlags |
| OS services | missing | C4 | S | various system abilities | implement the members over various system abilities<br>android.os.IBinder.shellCommand |
...
| Content & intents | hollow-candidate | C9 | verify | ability_runtime | check each hollow body against AOSP<br>android.content.Context.getAttributionTag, android.content.Context.isRestricted, android.content.ServiceConnection.onNullBinding |
...
| WebView | hollow-candidate | C9 | verify | web (ArkWeb) or bundled Chromium | check each hollow body against AOSP<br>android.webkit.WebSettings.getForceDark, android.webkit.WebSettings.setForceDark |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>B, androidx.sharetarget.ShortcutInfoCompatSaverImpl, androidx.window.extensions.WindowExtensions |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| keyguard | null | C4 | M | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>5 call sites, e.g. androidx.biometric.utils.KeyguardUtils$Api23Impl.getKeyguardManager |
| phone | inert | C4 | M | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>1 call sites, e.g. io.github.muntashirakon.AppManager.usage.AppUsageStatsManager.getSubscriberIds |
...
| jobscheduler | hollow | C9 | S | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>1 call sites, e.g. androidx.core.app.JobIntentService$JobWorkEnqueuer.<init> `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2162` |
| launcherapps | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. io.github.muntashirakon.AppManager.compat.PackageManagerCompat.getLaunchIntentForPackage |
| servicediscovery | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. io.github.muntashirakon.adb.android.AdbMdns.<init> |
...
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>1 call sites, e.g. io.github.muntashirakon.AppManager.self.filecache.InternalCacheCleanerService.scheduleAlarm |
...
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>5 call sites, e.g. androidx.appcompat.widget.AppCompatReceiveContentHelper.maybeHandleMenuActionViaPerformReceiveContent |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>3 call sites, e.g. io.github.muntashirakon.AppManager.adb.AdbUtils.isWifiConnected |
...
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>16 call sites, e.g. androidx.activity.ImmLeaksCleaner.onStateChanged `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>135 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>8 call sites, e.g. androidx.core.content.ContextCompat.checkSelfPermission |
...
| search | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. io.github.muntashirakon.AppManager.compat.ActivityManagerCompat.startActivityViaAssist |
| storage | supplied | C0 | verify | filemanagement/storage_service | none<br>1 call sites, e.g. io.github.muntashirakon.AppManager.compat.StorageManagerCompat.from |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Android keystore provider ("AndroidKeyStore") | supplied | C0 | verify | security/huks (OH Universal Keystore); keystore2 has no OH counterpart | install an "AndroidKeyStore" provider whose keys come from KeyGenParameterSpec and stay per app: software keys in app data (days), or OH HUKS for hardware-backed keys (weeks). A blocker wherever the app opens it at startup (secure storage, encrypted preferences, biometric crypto, attestation)<br>3 classes name it, e.g. Landroidx/biometric/utils/CryptoObjectUtils;, Lio/github/muntashirakon/AppManager/crypto/ks/CompatUtil;, Lio/github/muntashirakon/AppManager/misc/DeviceInfo2;; calls KeyGenerator.getInstance("AES"), KeyPairGenerator.getInstance("RSA"), KeyStore.getInstance("AndroidKeyStore") `framework/core/java/SoftwareAndroidKeyStore.java:65` |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Components in secondary processes: :audio_player | missing | C4 | L | appspawn (second process) | spawn and route secondary Android processes |
...
| Content providers installed at bind (2, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>FmProvider, InitializationProvider — probe: `probes/provider-manifest` |
| Component lookups return manifest <meta-data> | supplied | C0 | verify | none (answered from the APK inside Westlake) | apply updateFlagsForComponent semantics in the source-app PM path<br>6 components carry meta-data (0 directBootAware, e.g. ); app calls ['getActivityInfo', 'getApplicationInfo', 'getPackageInfo', 'getProviderInfo', 'getServiceInfo'] `framework/package-manager/java/SourcePackageRegistry.java:77` — probe: `probes/service-metadata` |
...
| android.os.SELinux: 12 of 12 natives unregistered | missing | C3 | M | the JNI half of the framework class (libandroid_runtime in AOSP) | port the AOSP JNI source for the class and register it at startup, before application bind<br>open: getFileContext(Ljava/io/FileDescriptor;)Ljava/lang/String;, getFileContext(Ljava/lang/String;)Ljava/lang/String;, isSELinuxEnabled()Z, setFileContext(Ljava/lang/String;Ljava/lang/String;)Z |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| libam.so → 3 classes, 3 members | supplied | C0 | none | through the Java framework | none<br>e.g. android/system/ErrnoException, android/system/StructPasswd, java/lang/String |
...
| Create lnk_file in app data | denied | C5 | M | SELinux u:r:normal_hap:s0 → u:object_r:appdat:s0 | neverallow'd for hap_domain on app data; needs a libc-level emulation of symlink(), not a policy rule<br>android.system.Os:symlink, java.nio.file.Files:createSymbolicLink `sepolicy/base/public/hap_domain.te neverallow hap_domain ~{ tmpfs_data_file dev_file ... data_user_file hmdfs ... }:lnk_file *` |

## Limits of this map

- 10 service requests use computed names and are not resolved statically.
- Java absences excluded by API level: {'absent-from-platform': 140, 'newer-than-reference': 18}.
