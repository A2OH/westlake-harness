# ws.xsoh.etar 1.0.57 → OpenHarmony: API shim gap map
...
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 11 | 11 | 9×verify, 2×M |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 24 | 7 | 1×verify, 4×S, 2×M |
...
| phone | inert | C4 | M | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>2 call sites, e.g. com.android.calendar.alerts.AlertReceiver.createCallActivityIntent |
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>2 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.onKeyUpPanel `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
...
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. com.android.calendar.settings.GeneralPreferences.onCreate |
...
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>26 call sites, e.g. androidx.appcompat.widget.TooltipCompatHandler.onHover |
| account | supplied | C0 | verify | account/os_account | none<br>2 call sites, e.g. com.android.calendar.CalendarController.refreshCalendars |
...
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>5 call sites, e.g. androidx.work.impl.utils.ForceStopRunnable.setAlarm |
...
| layout_inflater | supplied | C0 | verify | unmapped | none<br>85 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| search | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. com.android.calendar.CalendarController.launchSearch |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>FileProvider, CalendarRecentSuggestionsProvider, InitializationProvider — probe: `probes/provider-manifest` |
...
- 28 service requests use computed names and are not resolved statically.
- Java absences excluded by API level: {'absent-from-platform': 17, 'newer-than-reference': 18}.
