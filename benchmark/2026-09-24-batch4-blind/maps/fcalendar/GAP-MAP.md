# org.fossify.calendar 1.10.3 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 29 | 11 | 2×verify, 8×S, 1×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 6 | 6 | 2×S, 2×M, 2×L |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>6 call sites, e.g. a3.e.a `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2162` |
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. j.a0.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
...
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. z1.c.a0 |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites, e.g. rc.m.s |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>2 call sites, e.g. z1.c.Z `framework/package-manager/java/OHUserManager.java:82` |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. i2.m0.a |
| print | unresolved | CU | verify | print_service | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. tb.c.D `SystemServiceRegistry.java:893` |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. o.p0.a `SystemServiceRegistry.java:413` |
...
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. b2.a0.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. cc.k.a |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>4 call sites, e.g. d7.g.<init> |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>2 call sites, e.g. s6.c.c |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. u9.f.k |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. o.v.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>3 call sites, e.g. d7.e.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>16 call sites, e.g. a2.b.a `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>46 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. h5.f.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. j.a0.y |
| notification | supplied | C0 | verify | notification (ANS) | none<br>6 call sites, e.g. androidx.work.impl.foreground.SystemForegroundService.c |
| power | supplied | C0 | verify | powermgr | none<br>4 call sites, e.g. j.w.<init> |
| search | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. org.fossify.calendar.activities.SelectTimeZoneActivity.onCreate |
| shortcut | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. org.fossify.calendar.activities.MainActivity.onResume |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.a0.C |
| window | supplied | C0 | verify | window_manager | none<br>9 call sites, e.g. i2.e1.a `framework/core/java/OHServiceManager.java:96` |
