# org.kde.kdeconnect_tp 1.35.16 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 31 | 14 | 2×verify, 9×S, 3×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 5 | 5 | 1×S, 2×M, 2×L |
...
| Other Android | hollow-candidate | C9 | verify | unmapped | check each hollow body against AOSP<br>android.accessibilityservice.AccessibilityService.onDestroy, android.animation.Animator.cancel, android.animation.Animator.end |
...
| Networking | hollow-candidate | C9 | verify | netmanager | check each hollow body against AOSP<br>android.net.ConnectivityManager$NetworkCallback.onAvailable, android.net.ConnectivityManager$NetworkCallback.onLost |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.sharetarget.ShortcutInfoCompatSaverImpl, kotlin.reflect.jvm.internal.ReflectionFactoryImpl, org.apache.harmony.xnet.provider.jsse.OpenSSLSocketImpl |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | M | audio_framework | replace the hollow binder with an implementation over audio_framework<br>4 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.dispatchKeyEvent `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| phone | inert | C4 | M | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>13 call sites, e.g. com.android.mms.service_alt.DownloadRequest.execute |
| wifi | null | C4 | M | communication/wifi | Android WifiManager facade over communication/wifi<br>3 call sites, e.g. com.android.mms.service_alt.DownloadRequest.execute |
| camera | inert | C4 | S | multimedia/camera_framework | Android CameraManager facade over multimedia/camera_framework<br>1 call sites, e.g. org.kde.kdeconnect.plugins.findmyphone.FlashlightManager.<init> |
| download | inert | C4 | S | miscservices/download_server | Android DownloadManager facade over miscservices/download_server<br>1 call sites, e.g. org.kde.kdeconnect.plugins.share.CompositeReceiveFileJob.publishFile |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. org.kde.kdeconnect.plugins.notifications.NotificationsPlugin.onCreate |
| launcherapps | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. org.kde.kdeconnect.plugins.notifications.NotificationFilterActivity.lambda$onCreate$2 |
| role | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. org.kde.kdeconnect.ui.DefaultSmsAppAlertDialogFragment$1.onPositiveButtonClicked |
| sensor | inert | C4 | S | sensors | Android SensorManager facade over sensors<br>3 call sites, e.g. org.kde.kdeconnect.plugins.mousepad.MousePadActivity.onCreate |
| servicediscovery | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. org.kde.kdeconnect.backends.lan.MdnsDiscovery.<init> |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>1 call sites, e.g. org.kde.kdeconnect.plugins.notifications.NotificationFilterActivity.lambda$onCreate$2 `framework/package-manager/java/OHUserManager.java:82` |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. org.kde.kdeconnect.plugins.runcommand.RunCommandUrlActivity.onCreate |
| telephony_subscription_service | unresolved | CU | verify | telephony/core_service | trace the helper's binder; then treat as null, inert or supplied<br>6 call sites, e.g. com.android.mms.service_alt.MmsNetworkManager.<init> `TelephonyFrameworkInitializer.java:108` |
...
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>1 call sites, e.g. com.google.android.mms.util_alt.SqliteWrapper.isLowMemory |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>1 call sites, e.g. com.android.mms.transaction.RetryScheduler.setRetryAlarm |
| appops | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. kotlin.ResultKt.checkSelfPermission |
...
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>10 call sites, e.g. com.android.mms.service_alt.DownloadRequest.useWifi |
| display | supplied | C0 | verify | display_manager | none<br>1 call sites, e.g. org.kde.kdeconnect.plugins.inputdevicesreceiver.InputDevicesReceiverPlugin.onCreate |
...
| layout_inflater | supplied | C0 | verify | unmapped | none<br>36 call sites, e.g. androidx.appcompat.app.AlertController$AlertParams.<init> `SystemServiceRegistry.java:593` |
...
| power | supplied | C0 | verify | powermgr | none<br>4 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl$AutoTimeNightModeManager.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>5 call sites, e.g. androidx.core.os.BuildCompat$Api30Impl.getDynamicShortcuts |
| storage | supplied | C0 | verify | filemanagement/storage_service | none<br>1 call sites, e.g. org.kde.kdeconnect.plugins.sftp.SftpPlugin.onPacketReceived |
...
| Content providers installed at bind (3, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>FileProvider, InitializationProvider, MmsFileProvider — probe: `probes/provider-manifest` |
| Component lookups return manifest <meta-data> | supplied | C0 | verify | none (answered from the APK inside Westlake) | apply updateFlagsForComponent semantics in the source-app PM path<br>23 components carry meta-data (0 directBootAware, e.g. ); app calls ['getActivityInfo', 'getApplicationInfo', 'getPackageInfo', 'getProviderInfo', 'getServiceInfo'] `framework/package-manager/java/SourcePackageRegistry.java:77` — probe: `probes/service-metadata` |
...
- Java absences excluded by API level: {'absent-from-platform': 21, 'newer-than-reference': 10}.
