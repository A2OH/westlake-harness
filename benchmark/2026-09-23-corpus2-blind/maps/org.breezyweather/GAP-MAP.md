# org.breezyweather 6.2.2_freenet → OpenHarmony: API shim gap map
...
|---|---|---|---|---|
| Java framework API | APK dex references − Westlake boot jars, filtered by API level | 10 | 10 | 7×verify, 3×S |
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 22 | 6 | 1×verify, 3×S, 2×M |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.compose.ui.platform.AndroidCompositionLocals_androidKt, kotlin.reflect.jvm.internal.ReflectionFactoryImpl, nl.adaptivity.xmlutil.AndroidStreamingFactory |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>3 call sites, e.g. dz4.invoke `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2118` |
| notification | hollow | C9 | M | notification (ANS) | replace the hollow binder with an implementation over notification (ANS)<br>2 call sites, e.g. androidx.work.impl.foreground.SystemForegroundService.a `framework/android-runtime/src/AndroidRuntime.cpp:821` |
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>2 call sites, e.g. hq.w `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1599` |
| sensor | inert | C4 | S | sensors | Android SensorManager facade over sensors<br>3 call sites, e.g. pe0.S |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>1 call sites, e.g. up.H `framework/package-manager/java/OHUserManager.java:82` |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>2 call sites, e.g. er.o `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>14 call sites, e.g. ah3.r |
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>1 call sites, e.g. ih3.setCheckedState |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>3 call sites, e.g. bl4.a |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>1 call sites, e.g. u42.b |
| appops | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. qy3.h |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>2 call sites, e.g. mq.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>5 call sites, e.g. kw3.X |
| display | supplied | C0 | verify | display_manager | none<br>1 call sites, e.g. p4.b |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>11 call sites, e.g. or.onDetachedFromWindow `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>88 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. xp.c |
| location | supplied | C0 | verify | location | none<br>3 call sites, e.g. hq.A |
| power | supplied | C0 | verify | powermgr | none<br>4 call sites, e.g. dq.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. rr4.i |
| uimode | supplied | C0 | verify | display / theme | none<br>2 call sites, e.g. hq.F |
| window | supplied | C0 | verify | window_manager | none<br>15 call sites, e.g. a20.f `framework/core/java/OHServiceManager.java:96` |
...
| Content providers installed at bind (4, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>WeatherContentProvider, FileProvider, InitializationProvider, BypassProvider — probe: `probes/provider-manifest` |
...
| libsqlite3x.so → 23 classes, 27 members | supplied | C0 | none | through the Java framework | none<br>e.g. android/database/CharArrayBuffer, android/database/sqlite/SQLiteAbortException, android/database/sqlite/SQLiteAccessPermException, android/database/sqlite/SQLiteBindOrColumnIndexOutOfRangeException |
...
- Java absences excluded by API level: {'absent-from-platform': 4, 'newer-than-reference': 7}.
