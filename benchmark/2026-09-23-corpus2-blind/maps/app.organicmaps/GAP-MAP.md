# app.organicmaps 2026.08.27-18-FDroid → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 22 | 7 | 1×verify, 2×S, 4×M |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.work.impl.background.gcm.GcmScheduler, kotlin.reflect.jvm.internal.ReflectionFactoryImpl, org.bouncycastle.jsse.provider.BouncyCastleJsseProvider |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | M | audio_framework | replace the hollow binder with an implementation over audio_framework<br>2 call sites, e.g. androidx.appcompat.app.AppCompatDelegateImpl.onKeyUpPanel `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1599` |
| jobscheduler | hollow | C9 | M | resourceschedule/work_scheduler | replace the hollow binder with an implementation over resourceschedule/work_scheduler<br>3 call sites, e.g. androidx.work.impl.background.systemjob.JobSchedulerExtKt.createErrorMessage `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:2118` |
| notification | hollow | C9 | M | notification (ANS) | replace the hollow binder with an implementation over notification (ANS)<br>15 call sites, e.g. androidx.core.content.ContextCompat.checkSelfPermission `framework/android-runtime/src/AndroidRuntime.cpp:821` |
| sensor | inert | C4 | M | sensors | Android SensorManager facade over sensors<br>1 call sites, e.g. app.organicmaps.sdk.location.SensorHelper.<init> |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>3 call sites, e.g. app.organicmaps.widget.placepage.PlacePageUtils.copyToClipboard |
...
| display | supplied | C0 | verify | display_manager | none<br>2 call sites, e.g. androidx.car.app.CarContext.attachBaseContext |
...
| storage | supplied | C0 | verify | filemanagement/storage_service | none<br>1 call sites, e.g. app.organicmaps.sdk.settings.StoragePathManager.addStorageOption |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| liborganicmaps.so → 8 classes, 23 members | supplied | C0 | none | through the Java framework | none<br>e.g. android/util/Log, android/util/Pair, java/lang/Error, java/lang/Long |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| Create lnk_file in app data | denied | C5 | M | SELinux u:r:normal_hap:s0 → u:object_r:appdat:s0 | neverallow'd for hap_domain on app data; needs a libc-level emulation of symlink(), not a policy rule<br>liborganicmaps.so:symlink `sepolicy/base/public/hap_domain.te neverallow hap_domain ~{ tmpfs_data_file dev_file ... data_user_file hmdfs ... }:lnk_file *` |
...
- Java absences excluded by API level: {'newer-than-reference': 5}.
