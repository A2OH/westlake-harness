# org.fossify.notes 1.7.0 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 26 | 9 | 2×verify, 7×S |
...
| App framework | hollow-candidate | C9 | verify | ability_runtime | check each hollow body against AOSP<br>android.app.Activity.onActionModeFinished, android.app.Activity.onActionModeStarted, android.app.Activity.onActivityResult |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. j.A.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. s.o.b |
| fingerprint | null | C5 | S | unmapped | truthful local manager (feature absent)<br>2 call sites, e.g. B5.l.c |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. s.E.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. com.bumptech.glide.c.S |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites; Kotlin casts it non-null in 1 methods (e.g. o5.w.C): a null answer throws there, it is not skipped |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. J0.P.a |
| print | unresolved | CU | verify | print_service | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. u5.o.onPageFinished `SystemServiceRegistry.java:893` |
...
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. C0.z.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. B5.e.c |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. O2.f.<init> |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>3 call sites, e.g. com.bumptech.glide.e.I |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. com.bumptech.glide.d.j |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. J0.i.<init> |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. B5.l.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>15 call sites, e.g. A.a.n `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>46 call sites, e.g. M3.k.g `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. g2.e.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. j.A.y |
| notification | supplied | C0 | verify | notification (ANS) | none<br>1 call sites, e.g. u1.g.<init> |
| power | supplied | C0 | verify | powermgr | none<br>2 call sites, e.g. j.w.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. A5.n.onMenuItemClick |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.A.C |
| window | supplied | C0 | verify | window_manager | none<br>7 call sites, e.g. J0.j0.a `framework/core/java/OHServiceManager.java:96` |
