# org.fossify.clock 1.6.0 → OpenHarmony: API shim gap map
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>2 call sites, e.g. j.A.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. s.o.b |
| fingerprint | null | C5 | S | unmapped | truthful local manager (feature absent)<br>2 call sites, e.g. P2.e.b |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. s.E.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. f4.f.L |
| usb | null | C4 | S | usb_manager | Android UsbManager facade over usb_manager<br>1 call sites; Kotlin casts it non-null in 1 methods (e.g. I5.n.g): a null answer throws there, it is not skipped |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>2 call sites, e.g. J0.P.a |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. p.N.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>13 call sites, e.g. G1.T.h |
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. A2.x.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. A5.p0.a |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. P2.g.<init> |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>1 call sites, e.g. j5.e.h |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. f4.f.h |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. J0.i.<init> |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. P2.e.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>13 call sites, e.g. A.a.l `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>45 call sites, e.g. H5.Q.<init> `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. g5.a.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. j.A.y |
| notification | supplied | C0 | verify | notification (ANS) | none<br>2 call sites, e.g. f4.f.F |
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. j.w.<init> |
| shortcut | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. org.fossify.clock.activities.MainActivity.onResume |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.A.C |
| window | supplied | C0 | verify | window_manager | none<br>6 call sites, e.g. J0.i0.a `framework/core/java/OHServiceManager.java:96` |
