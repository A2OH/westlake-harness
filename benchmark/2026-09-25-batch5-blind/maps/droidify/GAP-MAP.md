# com.looker.droidify 0.7.7 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 21 | 5 | 1×verify, 2×S, 2×M |
| Keystore & crypto providers | JCA provider names the code selects → providers the Westlake runtime installs → OH HUKS | 0 | 0 | — |
| Package manager & manifest | manifest features and PackageManager calls → Westlake PM semantics | 6 | 5 | 1×verify, 3×S, 1×M |
...
| Job scheduling | hollow-candidate | C9 | verify | resourceschedule/work_scheduler | check each hollow body against AOSP<br>android.app.job.JobInfo.getMinFlexMillis, android.app.job.JobService.onCreate, android.app.job.JobService.onDestroy |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| camera | inert | C4 | M | multimedia/camera_framework | Android CameraManager facade over multimedia/camera_framework<br>2 call sites, e.g. androidx.cardview.widget.CardView$1.<init> |
...
| appops | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. androidx.core.os.HandlerCompat.checkSelfPermission |
...
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>4 call sites, e.g. androidx.camera.core.impl.Quirks.<init> |
| display | supplied | C0 | verify | display_manager | none<br>2 call sites, e.g. androidx.camera.camera2.internal.DisplayInfoManager.<init> |
...
| notification | supplied | C0 | verify | notification (ANS) | none<br>13 call sites, e.g. androidx.core.app.NotificationManagerCompat.<init> |
...
| Content providers installed at bind (2, initOrder honoured) | unverified | CU | verify | none | install every main-process provider in initOrder before Application.onCreate<br>ShizukuProvider, Cache$Provider — probe: `probes/provider-manifest` |
| Component lookups return manifest <meta-data> | supplied | C0 | verify | none (answered from the APK inside Westlake) | apply updateFlagsForComponent semantics in the source-app PM path<br>2 components carry meta-data (0 directBootAware, e.g. ); app calls ['getActivityInfo', 'getApplicationInfo', 'getPackageInfo', 'getServiceInfo'] `framework/package-manager/java/SourcePackageRegistry.java:77` — probe: `probes/service-metadata` |
...
- Java absences excluded by API level: {'absent-from-platform': 2, 'newer-than-reference': 6}.
