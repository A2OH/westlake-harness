# org.fossify.messages 1.9.1 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 28 | 11 | 2×verify, 8×S, 1×M |
...
| Framework natives | platform classes the app uses → their native methods → libraries the runtime registers them from | 4 | 4 | 3×M, 1×L |
...
| Other | probe-only | C8 | verify | unmapped | confirm the probed class should (not) exist on this platform<br>androidx.compose.ui.platform.AndroidCompositionLocals_androidKt, androidx.sharetarget.ShortcutInfoCompatSaverImpl, com.bumptech.glide.GeneratedAppGlideModuleImpl |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| phone | inert | C4 | M | telephony/core_service | Android TelephonyManager facade over telephony/core_service<br>11 call sites, e.g. d52.h |
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. yg.u `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. pp.b |
| fingerprint | null | C5 | S | unmapped | truthful local manager (feature absent)<br>2 call sites, e.g. com.github.ajalt.reprint.module.marshmallow.MarshmallowReprintModule.fingerprintManager |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. qf1.a |
| telecom | inert | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. lb1.k0 |
| user | strict | C9 | S | account/os_account | answer the methods callers use with Android's value for this device; a throw is never Android's answer (inside a JNI callback, WebView aborts on it)<br>1 call sites, e.g. lb1.j0 `framework/package-manager/java/OHUserManager.java:82` |
| vibrator | inert | C4 | S | sensors/miscdevice | Android Vibrator facade over sensors/miscdevice<br>1 call sites, e.g. e9.a |
| wifi | null | C4 | S | communication/wifi | Android WifiManager facade over communication/wifi<br>1 call sites, e.g. uj0.b |
| telephony_subscription_service | unresolved | CU | verify | telephony/core_service | trace the helper's binder; then treat as null, inert or supplied<br>3 call sites, e.g. g21.v0 `TelephonyFrameworkInitializer.java:108` |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. yh.a `SystemServiceRegistry.java:413` |
...
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. f8.<init> |
| account | supplied | C0 | verify | account/os_account | none<br>1 call sites, e.g. j80.t |
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. eu1.<init> |
| alarm | supplied | C0 | verify | time_service / reminder_agent | none<br>3 call sites, e.g. k4.l0 |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. uu2.v |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. dh.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>9 call sites, e.g. sh3.g |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>13 call sites, e.g. aj.onClick `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>45 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. hg.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. yg.y |
| notification | supplied | C0 | verify | notification (ANS) | none<br>2 call sites, e.g. c52.<init> |
| power | supplied | C0 | verify | powermgr | none<br>4 call sites, e.g. com.android.mms.transaction.TransactionService.b |
| shortcut | supplied | C0 | verify | unmapped | none<br>7 call sites, e.g. g21.x |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. yg.C |
| window | supplied | C0 | verify | window_manager | none<br>9 call sites, e.g. ec0.a `framework/core/java/OHServiceManager.java:96` |
...
- Java absences excluded by API level: {'newer-than-reference': 10, 'absent-from-platform': 4}.
