# org.fossify.filemanager 1.6.1 → OpenHarmony: API shim gap map
...
| System services | getSystemService name → AOSP fetcher/binder → Westlake provision → OH subsystem | 29 | 11 | 2×verify, 9×S |
...
| Item | Verdict | Class | Effort | OH touchpoint | Shim / evidence |
|---|---|---|---|---|---|
| audio | hollow | C9 | S | audio_framework | replace the hollow binder with an implementation over audio_framework<br>1 call sites, e.g. j.d0.w `framework/appspawn-x/java/com/android/internal/os/AppSpawnXInit.java:1627` |
| biometric | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. r.o.b |
| fingerprint | null | C5 | S | unmapped | truthful local manager (feature absent)<br>2 call sites, e.g. c7.m.b |
| keyguard | null | C4 | S | theme/screenlock | Android KeyguardManager facade over theme/screenlock<br>1 call sites, e.g. r.e0.a |
| storagestats | null | C5 | S | unmapped | truthful local manager (feature absent)<br>1 call sites, e.g. org.fossify.filemanager.fragments.StorageFragment.getVolumeStorageStats |
...
| print | unresolved | CU | verify | print_service | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. org.fossify.filemanager.activities.ReadTextActivity.createWebPrintJob `SystemServiceRegistry.java:893` |
| textclassification | unresolved | CU | verify | unmapped | trace the helper's binder; then treat as null, inert or supplied<br>1 call sites, e.g. androidx.appcompat.widget.z0.a `SystemServiceRegistry.java:413` |
| accessibility | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>12 call sites, e.g. androidx.core.view.d1.h |
| autofill | inert | C5 | none | unmapped | none: the manager is written to run without its service<br>3 call sites, e.g. a8.c.setCheckedState |
...
| activity | supplied | C0 | verify | ability_runtime (AMS) | none<br>2 call sites, e.g. androidx.room.a0.b |
| appops | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. androidx.room.m0.f |
| clipboard | supplied | C0 | verify | miscservices/pasteboard | none<br>3 call sites, e.g. androidx.appcompat.widget.b0.onTextContextMenuItem |
| connectivity | supplied | C0 | verify | netmanager (NetConnManager) | none<br>1 call sites, e.g. c7.m.get |
| input_method | supplied | C0 | verify | inputmethod_framework | none<br>14 call sites, e.g. a2.b.invoke `framework/core/java/OHServiceManager.java:105` |
| layout_inflater | supplied | C0 | verify | unmapped | none<br>58 call sites, e.g. androidx.appcompat.view.menu.ListMenuItemView.getInflater `SystemServiceRegistry.java:593` |
| locale | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. e5.f.run |
| location | supplied | C0 | verify | location | none<br>1 call sites, e.g. j.d0.A |
| notification | supplied | C0 | verify | notification (ANS) | none<br>2 call sites, e.g. org.fossify.commons.extensions.ContextKt.getNotificationManager |
| power | supplied | C0 | verify | powermgr | none<br>1 call sites, e.g. j.z.<init> |
| search | supplied | C0 | verify | unmapped | none<br>1 call sites, e.g. org.fossify.filemanager.activities.MimeTypesActivity.setupSearch |
| shortcut | supplied | C0 | verify | unmapped | none<br>2 call sites, e.g. org.fossify.commons.extensions.ContextKt.getShortcutManager |
| storage | supplied | C0 | verify | filemanagement/storage_service | none<br>2 call sites, e.g. org.fossify.filemanager.extensions.ContextKt.getAllVolumeNames |
| uimode | supplied | C0 | verify | display / theme | none<br>1 call sites, e.g. j.d0.E |
| window | supplied | C0 | verify | window_manager | none<br>8 call sites, e.g. n.t.a `framework/core/java/OHServiceManager.java:96` |
