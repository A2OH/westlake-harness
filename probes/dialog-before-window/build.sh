...
javac -source 8 -target 8 -Xlint:-options -cp "$ANDROID_JAR" \
    -d "$OUT/classes" \
    "$ROOT/src/org/westlake/probe/dialogorder/MainActivity.java"
...
"$TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-pass pass:android \
    --key-pass pass:android --out "$OUT/dialog-before-window-probe.apk" \
    "$OUT/probe-aligned.apk"
"$TOOLS/apksigner" verify --verbose "$OUT/dialog-before-window-probe.apk"
sha256sum "$OUT/dialog-before-window-probe.apk"
