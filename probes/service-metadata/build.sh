...
javac -source 8 -target 8 -Xlint:-options -cp "$ANDROID_JAR" \
    -d "$BUILD_TMP/classes" \
    "$ROOT/src/org/westlake/probe/servicemetadata/MetadataService.java" \
    "$ROOT/src/org/westlake/probe/servicemetadata/ProbeApplication.java" \
    "$ROOT/src/org/westlake/probe/servicemetadata/MainActivity.java"
...
"$TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-pass pass:android \
    --key-pass pass:android --out "$BUILD_TMP/service-metadata-probe.apk" \
    "$BUILD_TMP/probe-aligned.apk"
"$TOOLS/apksigner" verify --verbose "$BUILD_TMP/service-metadata-probe.apk"
cp "$BUILD_TMP/service-metadata-probe.apk" "$OUT/service-metadata-probe.apk"
sha256sum "$OUT/service-metadata-probe.apk"
