...
javac -source 8 -target 8 -Xlint:-options -cp "$ANDROID_JAR" \
    -d "$BUILD_TMP/classes" \
    "$ROOT/src/org/westlake/probe/wvb/MainActivity.java" \
...
"$TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-pass pass:android \
    --key-pass pass:android --out "$BUILD_TMP/webview-boundaries-probe.apk" \
    "$BUILD_TMP/probe-aligned.apk"
"$TOOLS/apksigner" verify --verbose "$BUILD_TMP/webview-boundaries-probe.apk"
cp "$BUILD_TMP/webview-boundaries-probe.apk" "$OUT/webview-boundaries-probe.apk"
sha256sum "$OUT/webview-boundaries-probe.apk"
