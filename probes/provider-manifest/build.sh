...
javac -source 8 -target 8 -Xlint:-options -cp "$ANDROID_JAR" \
    -d "$BUILD_TMP/classes" \
    "$ROOT/src/org/westlake/probe/providers/ProbeState.java" \
    "$ROOT/src/org/westlake/probe/providers/MainProbeProvider.java" \
    "$ROOT/src/org/westlake/probe/providers/RemoteProbeProvider.java" \
    "$ROOT/src/org/westlake/probe/providers/ProbeApplication.java" \
    "$ROOT/src/org/westlake/probe/providers/MainActivity.java"
...
"$TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-pass pass:android \
    --key-pass pass:android --out "$BUILD_TMP/provider-manifest-probe.apk" \
    "$BUILD_TMP/probe-aligned.apk"
"$TOOLS/apksigner" verify --verbose "$BUILD_TMP/provider-manifest-probe.apk"
cp "$BUILD_TMP/provider-manifest-probe.apk" "$OUT/provider-manifest-probe.apk"
sha256sum "$OUT/provider-manifest-probe.apk"
